...
一个原生 Windows 托盘小工具，实时显示 Codex 剩余额度和当前执行中的任务。高对比圆角方形托盘图标直接显示剩余百分比；额度卡片会同时列出并行任务、当前执行细节、已用时间、估算进度和预计剩余时间。
...
- 灰色 `?`：程序启动后尚未获得过有效数据
- 白色数字：实时数据正常
- 黑色数字：网络不可用、使用缓存或数据已过期；数字和红/黄/绿底色仍保留最近有效值
...
- 剩余量跨过 20%、10%、5% 时发出系统提醒，10 分钟内最多一次
